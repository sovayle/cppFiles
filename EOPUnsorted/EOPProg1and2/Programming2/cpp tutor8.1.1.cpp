...
void swap (int& x, int& argumentCount) {
    int temp = x;
    x = argumentCount;
    argumentCount = temp;
}
...
int main() {
    ...
    for (int i = 0; i < MAX_NUM - 1; ++i) {   				//i=0 i < 9 (does it 9 times)
        ...
        for (int j = i; j < MAX_NUM; ++j) {				//loop j = 0 for 9 times
            if (arr[j] < min_val) {						//checking for minimum value in list at j=1 3 is min so min ind is now 1 and vlaue is 3 for j=2 min ind is now 2 and value is 0
                min_ind = j;
                min_val = arr[j];
